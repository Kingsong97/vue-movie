...
const fetchMovies = async () => {
  try {
    const latestResponse = await axios.get(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=${apikey}&language=ko-KR`
    )

    comingMovies.value = latestResponse.data.results
  } catch (error) {
    console.log(error)
  }
}
...
<template>
  <h1 class="section-title">개봉예정</h1>
  <Swiper
  ...
  >
    <SwiperSlide v-for="(movie, index) in comingMovies" :key="movie.id">
